...
- **Автоматическое обновление** - синхронизация каждый час через Vercel Cron Jobs
...
- Для автоматического обновления используется Vercel Cron Job, который вызывает `/api/sync` каждый час (настроено в `vercel.json`).
- Endpoint `/api/sync` выполняет инкрементальную синхронизацию, загружая только новые изображения с момента последней синхронизации.
